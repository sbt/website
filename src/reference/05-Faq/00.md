...
#### What does sbt stand for, and how should I spell it?

sbt is just sbt: It's no longer an acronym. When the project was first started
it was called *Simple Build Tool*, but when the first release came out,
it was already called [sbt 0.3.2](http://www.scala-lang.org/old/node/392.html).

sbt is a proper noun that must be spelled with all lowercase.
Any deviation from the guideline such as title casing or all caps are subject to side-eye frown face.
We occationally set `sbt` in monospace font to specifically mean the bash/batch script `sbt` command.

So again, sbt does NOT stand for simple anything, and certainly not Scala Build Tool.
On the other hand, we are cool with [酢豚](https://ja.wikipedia.org/wiki/%E9%85%A2%E8%B1%9A) (subuta) as nickname.
