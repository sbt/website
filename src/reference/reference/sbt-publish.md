...
`sbt` \[_query_ / \] `publish`<br>
`sbt` \[_query_ / \] `publishSigned`<br>
`sbt` \[_query_ / \] `publishLocal`<br>
`sbt` \[_query_ / \] `publishM2`

Description
-----------

The publish family of tasks provide means for compiling and publishing your project.
_Publishing_ in this context consists of
uploading a descriptor, such as a Maven POM or `ivy.xml`, and artifacts,
such as a JAR or `war` file, to a repository so that other projects can specify
your project as a dependency.

- The `publish` task publishes your project to a remote repository,
  such as JFrog Artifactory or Sonatype Nexus instance.
- The `publishSigned` task, enabled using [sbt-pgp plugin][sbt-pgp],
  is used to publish GPG-signed artifacts.
- The `publishLocal` task publishes your project to
  the Ivy local file repository, which is usually located at `$HOME/.ivy2/local/`. You can then use this project from other projects on the same machine.
- The `publishM2` task publishes your project to the local Maven repository.

There's a specific recipe for [publishing to the Central Repo](../recipes/central.md).
...
If you're publishing the Central Repository, you will also have to select the
right repository depending on your artifacts: SNAPSHOT versions go to
the central-snapshots repository while other versions go to
the local staging repository.
Doing this selection can be done by using the value of the `version` setting:
...
~~~admonish warning
Generally the use of SNAPSHOT dependencies should be avoided beyond testing on a single machine
since it makes dependency resolution slower and the build non-repeatable.
~~~
...
To support multiple incompatible Scala versions, use projectMatrix
and publish (see [Cross building setup](./cross-building-setup.md)).
...
to change the `CrossVersion`.
See the [Cross building setup](./cross-building-setup.md) page for more
details under the _Publishing convention_ section.
...
and disable or modify the default artifacts.
See the [Artifact](./artifact.md) page for details.
...
This information will be annotated into the `pom.xml` file, which helps
downstream projects determine whether a version conflict is safe to resolve or not.
See [Preventing version conflicts with versionScheme](https://www.scala-lang.org/blog/2021/02/16/preventing-version-conflicts-with-versionscheme.html) (2021).
