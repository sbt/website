...
Abtract classes are less flexible compared to traits, but ease maintaining binary compatibility. They also have better Java interoperability.
...
The typeclass pattern with pure traits might be ease maintaining binary compatibility more so than subclassing.

#### Avoid case classes, use sbt-datatype

`case class` involves code generation that makes it harder to maintain binary compatibility over time.
...
#### Use specific data types (`Vector`, `List`, or `Array`), rather than `Seq`
...
#### Define typeclass instances in companion objects
